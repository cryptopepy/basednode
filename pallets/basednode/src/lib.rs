...
pub mod pallet {
    ...
    pub trait Config: frame_system::Config {
        ...
        // --- Currency type that will be used to place deposits on agents
        ...
        type InitialRAORecycledForRegistration: Get<u128>;
        #[pallet::constant] // Initial Burn.
        type InitialBurn: Get<u128>;
        #[pallet::constant] // Initial Max Burn.
        type InitialMaxBurn: Get<u128>;
        #[pallet::constant] // Initial Min Burn.
        type InitialMinBurn: Get<u128>;
        ...
        #[pallet::constant] // Initial pruning score for each agent.
        ...
        type InitialNetworkMinLockCost: Get<u128>;
        #[pallet::constant] // Initial network brain cut.
        type InitialBrainOwnerCut: Get<u16>;
        #[pallet::constant] // Initial lock reduction interval.
        type InitialNetworkLockReductionInterval: Get<u64>;
        #[pallet::constant] // Initial max allowed brains
        type InitialBrainLimit: Get<u16>;
        #[pallet::constant] // Initial network creation rate limit
        type InitialNetworkRateLimit: Get<u64>;
        #[pallet::constant] // Initial network brain cut.
        type InitialBrainOwnerByTokenBalanceCut: Get<u128>;
    }
    ...
    #[pallet::type_value]
    pub fn DefaultBlockEmission<T: Config>() -> u128 {
        8_000_000_000_000_000_000_000 // 8_000 $BASED
    }
    #[pallet::type_value]
    ...
    #[pallet::type_value]
    pub fn DefaultTotalIssuance<T: Config>() -> u128 {
    ...
    pub type BlockEmission<T> = StorageValue<_, u128, ValueQuery, DefaultBlockEmission<T>>;
    #[pallet::storage] // --- ITEM ( total_issuance )
    pub type TotalIssuance<T> = StorageValue<_, u128, ValueQuery, DefaultTotalIssuance<T>>;
    #[pallet::storage] // --- MAP ( hot ) --> stake | Returns the total amount of stake under a computekey.
    pub type TotalComputekeyStake<T: Config> =
        StorageMap<_, Identity, T::AccountId, u64, ValueQuery, DefaultAccountTake<T>>;
    #[pallet::storage] // --- MAP ( cold ) --> stake | Returns the total amount of stake under a personalkey.
    pub type TotalPersonalkeyStake<T: Config> =
        StorageMap<_, Identity, T::AccountId, u64, ValueQuery, DefaultAccountTake<T>>;
    #[pallet::storage] // --- MAP ( hot ) --> cold | Returns the controlling personalkey for a computekey.
    pub type Owner<T: Config> =
        StorageMap<_, Blake2_128Concat, T::AccountId, T::AccountId, ValueQuery, DefaultAccount<T>>;
    #[pallet::storage] // --- MAP ( hot ) --> take | Returns the computekey delegation take. And signals that this key is open for delegation.
    pub type Delegates<T: Config> =
        StorageMap<_, Blake2_128Concat, T::AccountId, u16, ValueQuery, DefaultDefaultTake<T>>;
    #[pallet::storage] // --- DMAP ( hot, cold ) --> stake | Returns the stake under a personalkey prefixed by computekey.
    ...
    #[pallet::type_value]
    pub fn DefaultBurn<T: Config>() -> u128 {
        T::InitialBurn::get()
    }
    #[pallet::type_value]
    pub fn DefaultMinBurn<T: Config>() -> u128 {
        T::InitialMinBurn::get()
    }
    #[pallet::type_value]
    pub fn DefaultMaxBurn<T: Config>() -> u128 {
    ...
    #[pallet::type_value]
    pub fn DefaultRAORecycledForRegistration<T: Config>() -> u128 {
    ...
    pub type Burn<T> = StorageMap<_, Identity, u16, u128, ValueQuery, DefaultBurn<T>>;
    ...
    pub type MinBurn<T> = StorageMap<_, Identity, u16, u128, ValueQuery, DefaultMinBurn<T>>;
    #[pallet::storage] // --- MAP ( netuid ) --> MaxBurn
    pub type MaxBurn<T> = StorageMap<_, Identity, u16, u128, ValueQuery, DefaultMaxBurn<T>>;
    ...
    pub type RAORecycledForRegistration<T> =
        StorageMap<_, Identity, u16, u128, ValueQuery, DefaultRAORecycledForRegistration<T>>;
    ...
    #[pallet::type_value]
    pub fn DefaultComputekeys<T: Config>() -> Vec<u16> {
        vec![]
    }
    #[pallet::type_value]
    ...
    #[pallet::type_value]
    pub fn DefaultNetworkMinLockCost<T: Config>() -> u128 {
    ...
    #[pallet::type_value]
    pub fn DefaultBrainOwnerCut<T: Config>() -> u16 {
        T::InitialBrainOwnerCut::get()
    }
    #[pallet::type_value]
    pub fn DefaultBrainLimit<T: Config>() -> u16 {
        T::InitialBrainLimit::get()
    }
    #[pallet::type_value]
    ...
    #[pallet::type_value]
    pub fn DefaultBrainOwnerByTokenBalanceCut<T: Config>() -> u128 {
        T::InitialBrainOwnerByTokenBalanceCut::get()
    }

    #[pallet::storage] // --- ITEM( total_number_of_existing_networks )
    pub type BrainLimit<T> = StorageValue<_, u16, ValueQuery, DefaultBrainLimit<T>>;
    #[pallet::storage] // --- ITEM( total_number_of_existing_networks )
    pub type TotalNetworks<T> = StorageValue<_, u16, ValueQuery>;
    #[pallet::storage] // --- MAP ( netuid ) --> brain_n (Number of UIDs in the network).
    pub type BrainN<T: Config> = StorageMap<_, Identity, u16, u16, ValueQuery, DefaultN<T>>;
    ...
    #[pallet::storage] // --- DMAP ( computekey, netuid ) --> bool
    ...
    pub type NetworkMinLockCost<T> = StorageValue<_, u128, ValueQuery, DefaultNetworkMinLockCost<T>>;
    #[pallet::storage] // ITEM( last_network_lock_cost )
    pub type NetworkLastLockCost<T> =
        StorageValue<_, u128, ValueQuery, DefaultNetworkMinLockCost<T>>;
    ...
    #[pallet::storage] // ITEM( brain_owner_cut )
    pub type BrainOwnerCut<T> = StorageValue<_, u16, ValueQuery, DefaultBrainOwnerCut<T>>;

    #[pallet::storage] // Map from brain id to ERC20 token address
    pub type BrainContract<T: Config> =
        StorageMap<_, Identity, u16, T::AccountId, ValueQuery, DefaultBrainOwner<T>>;


    #[pallet::storage]
    pub type BrainOwnerByTokenBalanceCut<T: Config> = StorageDoubleMap<
        Hasher1 = Identity, // no need to hash - bytes in AccountId20 are a good representation
        Key1 = T::AccountId, // ERC20 token address - // TODO(submissions) check if contract address follow all rules for AccountId20
        Hasher2 = Identity, // no need to hash - bytes in AccountId20 are a good representation
        Key2 = T::AccountId, // Owner adress
        Value = u128,
    ...
    #[pallet::type_value]
    pub fn DefaultBrainOwner<T: Config>() -> T::AccountId {
        T::AccountId::decode(&mut sp_runtime::traits::TrailingZeroInput::zeroes()).unwrap()
    }
    #[pallet::type_value]
    pub fn DefaultBrainLocked<T: Config>() -> u128 {
        0
    }
    #[pallet::type_value]
    ...
    #[pallet::storage]
    pub type BrainOwner<T: Config> =
        StorageMap<_, Identity, u16, T::AccountId, ValueQuery, DefaultBrainOwner<T>>;
    #[pallet::storage]
    pub type BrainLocked<T: Config> =
        StorageMap<_, Identity, u16, u128, ValueQuery, DefaultBrainLocked<T>>;

    // =================================
    // ==== Brainport / Promo Endpoints =====
    // =================================

    // --- Struct for Brainport.
    pub type BrainportInfoOf = BrainportInfo;

    #[derive(Encode, Decode, Default, TypeInfo, Clone, PartialEq, Eq, Debug)]
    pub struct BrainportInfo {
        pub block: u64,       // --- Brainport serving block.
        pub version: u32,     // --- Brainport version
        pub ip: u128,         // --- Brainport u128 encoded ip address of type v6 or v4.
        pub port: u16,        // --- Brainport u16 encoded port.
        pub ip_type: u8,      // --- Brainport ip type, 4 for ipv4 and 6 for ipv6.
        pub protocol: u8,     // --- Brainport protocol. TCP, UDP, other.
        pub placeholder1: u8, // --- Brainport proto placeholder 1.
        pub placeholder2: u8, // --- Brainport proto placeholder 1.
    }
    ...
    #[pallet::storage] // --- MAP ( netuid, computekey ) --> brainport_info
    pub(super) type Brainports<T: Config> =
        StorageDoubleMap<_, Identity, u16, Blake2_128Concat, T::AccountId, BrainportInfoOf, OptionQuery>;
    #[pallet::storage] // --- MAP ( netuid, computekey ) --> prometheus_info
    ...
    pub type AgentsToPruneAtNextEpoch<T: Config> = StorageMap<_, Identity, u16, u16, ValueQuery>;
    ...
    pub enum Event<T: Config> {
        ...
        NetworkCreateUpdate(u16, T::AccountId),
        NetworkRemoved(u16),    // --- Event created when a network is removed.
        StakeAdded(T::AccountId, u64), // --- Event created when stake has been transfered from the a personalkey account onto the computekey staking account.
        StakeRemoved(T::AccountId, u64), // --- Event created when stake has been removed from the computekey staking account onto the personalkey account.
        WeightsSet(u16, u16), // ---- Event created when a caller successfully sets their weights on a brain.
        AgentRegistered(u16, u16, T::AccountId), // --- Event created when a new agent account has been registered to the chain.
        BulkAgentsRegistered(u16, u16), // --- Event created when multiple uids have been concurrently registered.
        BulkBalancesSet(u16, u16),       // --- FIXME: Not used yet
        MaxAllowedUidsSet(u16, u16), // --- Event created when max allowed uids has been set for a brain.
        MaxWeightLimitSet(u16, u16), // --- Event created when the max weight limit has been set for a brain.
        DifficultySet(u16, u64), // --- Event created when the difficulty has been set for a brain.
        AdjustmentIntervalSet(u16, u16), // --- Event created when the adjustment interval is set for a brain.
        RegistrationPerIntervalSet(u16, u16), // --- Event created when registeration per interval is set for a brain.
        MaxRegistrationsPerBlockSet(u16, u16), // --- Event created when we set max registrations per block.
        ActivityCutoffSet(u16, u16), // --- Event created when an activity cutoff is set for a brain.
        RhoSet(u16, u16),            // --- Event created when Rho value is set.
        KappaSet(u16, u16),          // --- Event created when Kappa is set for a brain.
        MinAllowedWeightSet(u16, u16), // --- Event created when minimun allowed weight is set for a brain.
        ValidatorPruneLenSet(u16, u64), // --- Event created when the validator pruning length has been set.
        ScalingLawPowerSet(u16, u16), // --- Event created when the scaling law power has been set for a brain.
        WeightsSetRateLimitSet(u16, u64), // --- Event created when weights set rate limit has been set for a brain.
        ImmunityPeriodSet(u16, u16), // --- Event created when immunity period is set for a brain.
        BondsMovingAverageSet(u16, u64), // --- Event created when bonds moving average is set for a brain.
        MaxAllowedValidatorsSet(u16, u16), // --- Event created when setting the max number of allowed validators on a brain.
        BrainportServed(u16, T::AccountId), // --- Event created when the brainport server information is added to the network.
        ...
        DelegateAdded(T::AccountId, T::AccountId, u16), // --- Event created to signal that a computekey has become a delegate.
        ...
        BurnSet(u16, u128),             // --- Event created when setting burn on a network.
        MaxBurnSet(u16, u128),          // --- Event created when setting max burn on a network.
        MinBurnSet(u16, u128),          // --- Event created when setting min burn on a network.
        TxRateLimitSet(u64),           // --- Event created when setting the transaction rate limit.
        Sudid(DispatchResult),         // --- Event created when a sudo call is done.
        RegistrationAllowed(u16, bool), // --- Event created when registration is allowed/disallowed for a brain.
        PowRegistrationAllowed(u16, bool), // --- Event created when POW registration is allowed/disallowed for a brain.
        TempoSet(u16, u16),             // --- Event created when setting tempo on a network
        RAORecycledForRegistrationSet(u16, u128), // Event created when setting the RAO recycled for registration.
        SenateRequiredStakePercentSet(u64), // Event created when setting the minimum required stake amount for senate registration.
        AdjustmentAlphaSet(u16, u64), // Event created when setting the adjustment alpha on a brain.
        Faucet(T::AccountId, u64), // Event created when the facuet it called on the test net.
        BrainOwnerCutSet(u16),    // Event created when the brain owner cut is set.
        ...
        NetworkMinLockCostSet(u128), // Event created when the network minimum locking cost is set.
        BrainLimitSet(u16), // Event created when the maximum number of brains is set
        NetworkLockCostReductionIntervalSet(u64), // Event created when the lock cost reduction is set
        BrainOwnerContractCreated(u16, T::AccountId),
		BrainContractBalanceChanged(T::AccountId, T::AccountId, T::AccountId, u128),
        ComputekeySwapped{personalkey: T::AccountId, old_computekey: T::AccountId, new_computekey: T::AccountId} // Event created when a computekey is swapped
    }
    ...
    pub enum Error<T> {
        ...
        InvalidIpType, // ---- Thrown when the user tries to serve an brainport which is not of type 4 (IPv4) or 6 (IPv6).
        ...
        NotRegistered, // ---- Thrown when the caller requests setting or removing data from a agent which does not exist in the active set.
        NonAssociatedpersonalkey, // ---- Thrown when a stake, unstake or subscribe request is made by a personalkey which is not associated with the computekey account.
        ...
        BalanceWithdrawalError, // ---- Thrown when the caller tries to add stake, but for some reason the requested amount could not be withdrawn from the personalkey account.
        ...
        AlreadyRegistered, // ---- Thrown when the caller requests registering a agent which already exists in the active set.
        ...
        StakeAlreadyAdded, // --- Thrown when the caller requests adding stake for a computekey to the total stake which already added.
        ...
        AlreadyDelegate, // --- Thrown if the computekey attempts to become delegate when they are already.
        SettingWeightsTooFast, // --- Thrown if the computekey attempts to set weights twice within net_tempo/2 blocks.
        IncorrectNetworkVersionKey, // --- Thrown when a validator attempts to set weights from a validator with incorrect code base key.
        ServingRateLimitExceeded, // --- Thrown when an brainport or prometheus serving exceeds the rate limit for a registered agent.
        ...
        InvalidNetuid, // --- Thrown when invalid netuid is used
        ...
        ComputekeyOriginMismatch, // --- Thrown when the computekey passed is not the origin, but it should be
        // Senate errors
        SenateMember, // --- Thrown when attempting to do something to a senate member that is limited
        NotSenateMember, // --- Thrown when a computekey attempts to do something only senate members can do
        AlreadySenateMember, // --- Thrown when a computekey attempts to join the senate while already being a member
        BelowStakeThreshold, // --- Thrown when a computekey attempts to join the senate without enough stake
        NotDelegate, // --- Thrown when a computekey attempts to join the senate without being a delegate first
        ...
        NotBrainOwner,
        OperationNotPermittedonRootBrain,
        StakeTooLowForRoot, // --- Thrown when a computekey attempts to join the root brain with too little stake
        AllNetworksInImmunity, // --- Thrown when all brains are in the immunity period
        NotEnoughBalance,
    }
    ...
    impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
        fn build(&self) {
            ...
            for (personalkey, computekeys) in self.stakes.iter() {
                for (computekey, stake_uid) in computekeys.iter() {
                    ...
                    Keys::<T>::insert(netuid, uid, computekey.clone()); // Make computekey - uid association.
                    Uids::<T>::insert(netuid, computekey.clone(), uid); // Make uid - computekey association.
                    BlockAtRegistration::<T>::insert(netuid, uid, 0); // Fill block at registration.
                    IsNetworkMember::<T>::insert(computekey.clone(), netuid, true); // Fill network is member.

                    // Fill stake information.
                    Owner::<T>::insert(computekey.clone(), personalkey.clone());

                    TotalComputekeyStake::<T>::insert(computekey.clone(), stake);
                    TotalPersonalkeyStake::<T>::insert(
                        personalkey.clone(),
                        TotalPersonalkeyStake::<T>::get(personalkey).saturating_add(*stake),
                    );

                    // Update total issuance value
					let s = *stake;
					let stake_as_u128: u128 = (*stake).into();
                    TotalIssuance::<T>::put(TotalIssuance::<T>::get().saturating_add(stake_as_u128));

                    Stake::<T>::insert(computekey.clone(), personalkey.clone(), stake);

                    next_uid += 1;
                }
            }

            // Set correct length for Brain agents
            BrainN::<T>::insert(netuid, next_uid);
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn on_runtime_upgrade() -> frame_support::weights::Weight {
            ...
            // Hex encoded foundation personalkey
            ...
            weight = weight
                ...
                .saturating_add(migration::migrate_delete_brain_3::<T>())
                .saturating_add(migration::migrate_delete_brain_21::<T>());
    ...
    impl<T: Config> Pallet<T> {
        ...
        // made from the computekey account so is potentially insecure, however, the damage
        ...
        //      - The caller, a computekey who wishes to set their weights.
        //
        //  * `netuid` (u16):
        ...
        pub fn become_delegate(origin: OriginFor<T>, computekey: T::AccountId) -> DispatchResult {
            Self::do_become_delegate(origin, computekey, Self::get_default_take())
        }

        // --- Adds stake to a computekey. The call is made from the
        // personalkey account linked in the computekey.
        // Only the associated personalkey is allowed to make staking and
        // unstaking requests. This protects the agent against
        // attacks on its computekey running in production code.
        //
        // # Args:
        //  * 'origin': (<T as frame_system::Config>Origin):
        //      - The signature of the caller's personalkey.
        ...
        pub fn add_stake(
            ...
            Self::do_add_stake(origin, computekey, amount_staked)
        }

        // ---- Remove stake from the staking account. The call must be made
        // from the personalkey account attached to the agent metadata. Only this key
        ...
        pub fn remove_stake(
            ...
            Self::do_remove_stake(origin, computekey, amount_unstaked)
        }

        // ---- Serves or updates brainport /promethteus information for the agent associated with the caller. If the caller is
        ...
        // ---- Registers a new agent to the brain.
        //
        // # Args:
        //  * 'origin': (<T as frame_system::Config>Origin):
        ...
        pub fn register(
            ...
            Self::do_registration(origin, netuid, block_number, nonce, work, computekey, personalkey)
        }
        ...
        pub fn root_register(origin: OriginFor<T>, computekey: T::AccountId) -> DispatchResult {
            Self::do_root_register(origin, computekey)
        }
        ...
        pub fn burned_register(
            ...
            Self::do_burned_registration(origin, netuid, computekey)
        }

        #[pallet::call_index(70)]
        #[pallet::weight((0, DispatchClass::Operational, Pays::No))]
        pub fn swap_computekey(origin: OriginFor<T>, computekey: T::AccountId, new_computekey: T::AccountId) -> DispatchResultWithPostInfo {
            Self::do_swap_computekey(origin, &computekey, &new_computekey)
        }
        ...
        pub fn vote(
            ...
            Self::do_vote_root(origin, &computekey, proposal, index, approve)
        }
        ...
        .saturating_add(T::DbWeight::get().reads(16))
        .saturating_add(T::DbWeight::get().writes(28)), DispatchClass::Operational, Pays::No))]
        pub fn create_or_update_brain(origin: OriginFor<T>, netuid: u16, owner: T::AccountId) -> DispatchResult {
            Self::create_or_update_network_ownership(origin, netuid, owner)
        }

		// TODO(brain emissions) check / calculate weight and check if needed - sudo call doesn't
		// need to pay with Pays::no
        #[pallet::call_index(64)]
        #[pallet::weight((Weight::from_ref_time(85_000_000)
        .saturating_add(T::DbWeight::get().reads(16))
        .saturating_add(T::DbWeight::get().writes(28)), DispatchClass::Operational, Pays::No))]
        pub fn brain_owner_contract_created(origin: OriginFor<T>, netuid: u16, contract: T::AccountId) -> DispatchResult {
            Self::brain_contract_created(origin, netuid, contract)
        }
        ...
        .saturating_add(T::DbWeight::get().reads(16))
        .saturating_add(T::DbWeight::get().writes(28)), DispatchClass::Operational, Pays::No))]
        pub fn brain_contract_balance(origin: OriginFor<T>, contract: T::AccountId, from: T::AccountId, to: T::AccountId, balance: u128) -> DispatchResult {
            Self::brain_contract_balance_change(origin, contract, from, to, balance)
        }
    }

    // ---- Basednode helper functions.
    impl<T: Config> Pallet<T> {
        // --- Returns the transaction priority for setting weights.
        pub fn get_priority_set_weights(computekey: &T::AccountId, netuid: u16) -> u64 {
            if Uids::<T>::contains_key(netuid, &computekey) {
                let uid = Self::get_uid_for_net_and_computekey(netuid, &computekey.clone()).unwrap();
...
pub struct BasednodeSignedExtension<T: Config + Send + Sync + TypeInfo>(pub PhantomData<T>);

impl<T: Config + Send + Sync + TypeInfo> BasednodeSignedExtension<T>
where
...
impl<T: Config + Send + Sync + TypeInfo> sp_std::fmt::Debug for BasednodeSignedExtension<T> {
    fn fmt(&self, f: &mut sp_std::fmt::Formatter) -> sp_std::fmt::Result {
        write!(f, "BasednodeSignedExtension")
    }
}

impl<T: Config + Send + Sync + TypeInfo> SignedExtension for BasednodeSignedExtension<T>
where
    T::RuntimeCall: Dispatchable<Info = DispatchInfo, PostInfo = PostDispatchInfo>,
    <T as frame_system::Config>::RuntimeCall: IsSubType<Call<T>>,
{
    const IDENTIFIER: &'static str = "BasednodeSignedExtension";
    ...
    fn validate(
        ...
        match call.is_sub_type() {
            ...
            // })
            // ,
            Some(Call::create_or_update_brain { .. }) => {
                log::debug!("Call::create_or_update_brain!");
                Ok(ValidTransaction {
                    priority: Self::get_priority_vanilla(),
                    ..Default::default()
                })
    ...
    fn pre_dispatch(
        ...
        match call.is_sub_type() {
            ...
            // }
            Some(Call::create_or_update_brain { netuid, owner }) => {
                log::debug!("pre_dispatch create_or_update_brain netuid = {:?} owner = {:?}",
                    netuid, owner);
                let transaction_fee = 0;
                Ok((CallType::CreateOrUpdateNetwork, transaction_fee, who.clone()))
            }
            _ => {
                let transaction_fee = 0;
    ...
    fn post_dispatch(
        ...
        if let Some((call_type, _transaction_fee, _who)) = maybe_pre {
            match call_type {
                ...
                CallType::CreateOrUpdateNetwork => {
                    log::debug!("Not Implemented! CreateOrUpdateNetwork");
                }
                _ => {
                    log::debug!("Not Implemented!");
                }
...
pub trait CollectiveInterface<AccountId, Hash, ProposalIndex> {
    /// Remove vote
    fn remove_votes(computekey: &AccountId) -> Result<bool, DispatchError>;
