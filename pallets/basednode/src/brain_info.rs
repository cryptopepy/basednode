...
pub struct BrainHyperparams {
    ...
    min_burn: Compact<u128>,
    max_burn: Compact<u128>,
...
impl<T: Config> Pallet<T> {
    pub fn get_brain_info(netuid: u16) -> Option<BrainInfo<T>> {
        ...
        let brain_n = Self::get_brain_n(netuid);
        ...
        let burn: Compact<u128> = Self::get_burn_as_u64(netuid).into();
        ...
        return Some(BrainInfo {
            ...
            burn,
            owner: Self::get_brain_owner(netuid).into(),
        });
    }

    pub fn get_brains_info() -> Vec<Option<BrainInfo<T>>> {
        let mut brain_netuids = Vec::<u16>::new();
        let mut max_netuid: u16 = 0;
        for (netuid, added) in <NetworksAdded<T> as IterableStorageMap<u16, bool>>::iter() {
            if added {
                brain_netuids.push(netuid);
        ...
        let mut brains_info = Vec::<Option<BrainInfo<T>>>::new();
        for netuid_ in 0..(max_netuid + 1) {
            if brain_netuids.contains(&netuid_) {
                brains_info.push(Self::get_brain_info(netuid_));
            }
        }
    ...
    pub fn get_brain_hyperparams(netuid: u16) -> Option<BrainHyperparams> {
        if !Self::if_brain_exist(netuid) {
            return None;
        }
