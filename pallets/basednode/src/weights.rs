...
impl<T: Config> Pallet<T> {
    ...
    pub fn do_set_weights(
        ...
        // --- 1. Check the caller's signature. This is the computekey of a registered account.
        let computekey = ensure_signed(origin)?;
        log::info!(
        ...
        // For the root network this number is the number of brains.
        ...
        // --- 5. Check to see if the computekey is registered to the passed network.
        ensure!(
            Self::is_computekey_registered_on_network(netuid, &computekey),
        ...
        // --- 7. Get the agent uid of associated computekey on network netuid.
        let agent_uid;
        let net_agent_uid = Self::get_uid_for_net_and_computekey(netuid, &computekey);
        ensure!(
            net_agent_uid.is_ok(),
            net_agent_uid
                .err()
        ...
        agent_uid = net_agent_uid.unwrap();

        // --- 8. Ensure the uid is not setting weights faster than the weights_set_rate_limit.
        let current_block: u64 = Self::get_current_block_as_u64();
        ensure!(
            Self::check_rate_limit(netuid, agent_uid, current_block),
            Error::<T>::SettingWeightsTooFast
        );

        // --- 9. Check that the agent uid is an allowed validator permitted to set non-self weights.
        if netuid != Self::get_root_netuid() {
            ensure!(
                Self::check_validator_permit(netuid, agent_uid, &uids, &values),
        ...
        // --- 12. Ensure that the weights have the required length.
            ...
            Self::check_length(netuid, agent_uid, &uids, &values),
        ...
        // --- 14. Ensure the weights are max weight limited
            ...
            Self::max_weight_limited(netuid, agent_uid, &uids, &max_upscaled_weights),
        ...
        Weights::<T>::insert(netuid, agent_uid, zipped_weights);

        // --- 17. Set the activity for the weights on this network.
        Self::set_last_update_for_uid(netuid, agent_uid, current_block);

        // --- 18. Emit the tracking event.
        log::info!(
            "WeightsSet( netuid:{:?}, agent_uid:{:?} )",
            netuid,
            agent_uid
        );
        Self::deposit_event(Event::WeightsSet(netuid, agent_uid));
    ...
    // Checks if the agent has set weights within the weights_set_rate_limit.
    //
    pub fn check_rate_limit(netuid: u16, agent_uid: u16, current_block: u64) -> bool {
        if Self::is_uid_exist_on_network(netuid, agent_uid) {
            ...
            let last_set_weights: u64 = Self::get_last_update_for_uid(netuid, agent_uid);
    ...
    pub fn check_len_uids_within_allowed(netuid: u16, uids: &Vec<u16>) -> bool {
        let brain_n: u16 = Self::get_brain_n(netuid);
        // we should expect at most brain_n uids.
        return uids.len() <= brain_n as usize;
    }
}
