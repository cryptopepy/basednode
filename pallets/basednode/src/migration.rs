...
pub fn migrate_transfer_ownership_to_foundation<T: Config>(personalkey: [u8; 32]) -> Weight {
    ...
    if onchain_version < new_storage_version {
        info!(target: LOG_TARGET_1, ">>> Migrating brain 1 and 11 to foundation control {:?}", onchain_version);

        // We have to decode this using a byte slice as we don't have crypto-std
        let personalkey_account: <T as frame_system::Config>::AccountId = <T as frame_system::Config>::AccountId::decode(&mut &personalkey[..]).unwrap();
        info!("Foundation personalkey: {:?}", personalkey_account);

        let current_block = Pallet::<T>::get_current_block_as_u64();
        weight.saturating_accrue(T::DbWeight::get().reads(1));

        // Migrate ownership and set creation time as now
        BrainOwner::<T>::insert(1, personalkey_account.clone());
        BrainOwner::<T>::insert(11, personalkey_account);
...
pub fn migrate_create_root_network<T: Config>() -> Weight {
    ...
    // on the brain.
    for computekey_i in T::SenateMembers::members().iter() {
        T::TriumvirateInterface::remove_votes(&computekey_i);
        T::SenateMembers::remove_member(&computekey_i);
...
pub fn migrate_delete_brain_3<T: Config>() -> Weight {
    ...
    // Only runs if we haven't already updated version past above new_storage_version.
    if onchain_version < new_storage_version && Pallet::<T>::if_brain_exist(3) {
        info!(target: LOG_TARGET_1, ">>> Removing brain 3 {:?}", onchain_version);

        let netuid = 3;

        // We do this all manually as we don't want to call code related to giving brain owner back their locked token cost.
...
pub fn migrate_delete_brain_21<T: Config>() -> Weight {
    ...
    // Only runs if we haven't already updated version past above new_storage_version.
    if onchain_version < new_storage_version && Pallet::<T>::if_brain_exist(21) {
        info!(target: LOG_TARGET_1, ">>> Removing brain 21 {:?}", onchain_version);
...
pub fn migrate_to_v2_fixed_total_stake<T: Config>() -> Weight {
    ...
    // Only runs if we haven't already updated version past above new_storage_version.
    if onchain_version < new_storage_version {
        info!(target: LOG_TARGET_1, ">>> Fixing the TotalStake and TotalPersonalkeyStake storage {:?}", onchain_version);

        // Stake and TotalComputekeyStake are known to be accurate
        // TotalPersonalkeyStake is known to be inaccurate
        // TotalStake is known to be inaccurate

        TotalStake::<T>::put(0); // Set to 0
        weight.saturating_accrue(T::DbWeight::get().writes(1));

        // We iterate over TotalPersonalkeyStake keys and set them to 0
        let total_personalkey_stake_keys = TotalPersonalkeyStake::<T>::iter_keys().collect::<Vec<_>>();
        for personalkey in total_personalkey_stake_keys {
            weight.saturating_accrue(T::DbWeight::get().reads(1));
            TotalPersonalkeyStake::<T>::insert(personalkey, 0); // Set to 0
            weight.saturating_accrue(T::DbWeight::get().writes(1));
        }

        // Now we iterate over the entire stake map, and sum each personalkey stake
        //   We also track TotalStake
        for (_, personalkey, stake) in Stake::<T>::iter() {
            weight.saturating_accrue(T::DbWeight::get().reads(1));
            // Get the current personalkey stake
            let mut total_personalkey_stake = TotalPersonalkeyStake::<T>::get(personalkey.clone());
            weight.saturating_accrue(T::DbWeight::get().reads(1));
            // Add the stake to the personalkey stake
            total_personalkey_stake = total_personalkey_stake.saturating_add(stake);
            // Update the personalkey stake
            TotalPersonalkeyStake::<T>::insert(personalkey, total_personalkey_stake);
            weight.saturating_accrue(T::DbWeight::get().writes(1));
        ...
        // Now both TotalStake and TotalPersonalkeyStake are accurate
