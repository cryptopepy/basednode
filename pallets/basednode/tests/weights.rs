...
// Test the call passes through the basednode module.
#[test]
#[cfg(not(tarpaulin))]
fn test_set_weights_dispatch_info_ok() {
    new_test_ext().execute_with(|| {
        ...
        let call = RuntimeCall::BasedNode(BasednodeCall::set_weights {
            netuid,
            dests,
            weights,
            version_key,
...
fn test_weights_err_no_validator_permit() {
    new_test_ext().execute_with(|| {
        let computekey_account_id = U256::from(55);
        let netuid: u16 = 1;
        let tempo: u16 = 13;
        ...
        register_ok_agent(netuid, computekey_account_id, U256::from(66), 0);
        register_ok_agent(netuid, U256::from(1), U256::from(1), 65555);
        register_ok_agent(netuid, U256::from(2), U256::from(2), 75555);

        let weights_keys: Vec<u16> = vec![1, 2];
        let weight_values: Vec<u16> = vec![1, 2];
        let result = BasedNode::set_weights(
            RuntimeOrigin::signed(computekey_account_id),
            netuid,
            weights_keys,
            weight_values,
            0,
        ...
        let weight_values: Vec<u16> = vec![1, 2];
        let agent_uid: u16 =
            BasedNode::get_uid_for_net_and_computekey(netuid, &computekey_account_id)
                .expect("Not registered.");
        BasedNode::set_validator_permit_for_uid(netuid, agent_uid, true);
        let result = BasedNode::set_weights(
            RuntimeOrigin::signed(computekey_account_id),
            netuid,
            weights_keys,
...
fn test_weights_version_key() {
    new_test_ext().execute_with(|| {
        let computekey = U256::from(55);
        let personalkey = U256::from(66);
        let netuid0: u16 = 1;
        let netuid1: u16 = 2;
        add_network(netuid0, 0, 0);
        add_network(netuid1, 0, 0);
        register_ok_agent(netuid0, computekey, personalkey, 2143124);
        register_ok_agent(netuid1, computekey, personalkey, 3124124);

        let weights_keys: Vec<u16> = vec![0];
        let weight_values: Vec<u16> = vec![1];
        assert_ok!(BasedNode::set_weights(
            RuntimeOrigin::signed(computekey),
            netuid0,
            weights_keys.clone(),
            weight_values.clone(),
            0
        ...
        let key0: u64 = 12312;
        let key1: u64 = 20313;
        BasedNode::set_weights_version_key(netuid0, key0);
        BasedNode::set_weights_version_key(netuid1, key1);
    ...
    new_test_ext().execute_with(|| {
        ...
        let agent_uid: u16 =
            BasedNode::get_uid_for_net_and_computekey(netuid, &computekey_account_id)
                .expect("Not registered.");
        BasedNode::set_validator_permit_for_uid(netuid, agent_uid, true);
        BasedNode::set_weights_set_rate_limit(netuid, 10);
        assert_eq!(BasedNode::get_weights_set_rate_limit(netuid), 10);
...
fn test_weights_err_weights_vec_not_equal_size() {
    new_test_ext().execute_with(|| {
        ...
        let tempo: u16 = 13;
        add_network(netuid, tempo, 0);
        register_ok_agent(1, computekey_account_id, U256::from(66), 0);
        let agent_uid: u16 =
            BasedNode::get_uid_for_net_and_computekey(netuid, &computekey_account_id)
                .expect("Not registered.");
        BasedNode::set_validator_permit_for_uid(netuid, agent_uid, true);
...
fn test_weights_err_has_duplicate_ids() {
    new_test_ext().execute_with(|| {
        let computekey_account_id = U256::from(666);
        let netuid: u16 = 1;
        let tempo: u16 = 13;
        add_network(netuid, tempo, 0);

        BasedNode::set_max_allowed_uids(netuid, 100); // Allow many registrations per block.
        BasedNode::set_max_registrations_per_block(netuid, 100); // Allow many registrations per block.
        BasedNode::set_target_registrations_per_interval(netuid, 100); // Allow many registrations per block.
        // uid 0
        register_ok_agent(netuid, computekey_account_id, U256::from(77), 0);
        let agent_uid: u16 =
            BasedNode::get_uid_for_net_and_computekey(netuid, &computekey_account_id)
                .expect("Not registered.");
        BasedNode::set_validator_permit_for_uid(netuid, agent_uid, true);
        ...
        // uid 1
        register_ok_agent(netuid, U256::from(1), U256::from(1), 100_000);
        BasedNode::get_uid_for_net_and_computekey(netuid, &U256::from(1))
            .expect("Not registered.");

        // uid 2
        register_ok_agent(netuid, U256::from(2), U256::from(1), 200_000);
        BasedNode::get_uid_for_net_and_computekey(netuid, &U256::from(2))
            .expect("Not registered.");

        // uid 3
        register_ok_agent(netuid, U256::from(3), U256::from(1), 300_000);
        BasedNode::get_uid_for_net_and_computekey(netuid, &U256::from(3))
            .expect("Not registered.");

        assert_eq!(BasedNode::get_brain_n(netuid), 4);
...
fn test_weights_err_max_weight_limit() {
    ...
    new_test_ext().execute_with(|| {
        ...
        // Set params.
        BasedNode::set_max_allowed_uids(netuid, 5);
        BasedNode::set_target_registrations_per_interval(netuid, 5);
        BasedNode::set_max_weight_limit(netuid, u16::MAX / 5);
        BasedNode::set_min_allowed_weights(netuid, 0);

        // Add 5 accounts.
        println!("+Registering: net:{:?}, cold:{:?}, hot:{:?}", netuid, 0, 0);
        register_ok_agent(netuid, U256::from(0), U256::from(0), 55555);
        let agent_uid: u16 = BasedNode::get_uid_for_net_and_computekey(netuid, &U256::from(0))
            .expect("Not registered.");
        BasedNode::set_validator_permit_for_uid(netuid, agent_uid, true);
        ...
        assert_eq!(BasedNode::get_brain_n(netuid), 2);
        step_block(1);
        ...
        register_ok_agent(netuid, U256::from(3), U256::from(3), 95555);
        assert!(BasedNode::is_computekey_registered_on_network(
            netuid,
            &U256::from(3)
        ));
        ...
        register_ok_agent(netuid, U256::from(4), U256::from(4), 35555);
        assert!(BasedNode::is_computekey_registered_on_network(
            netuid,
            &U256::from(4)
        ));
        assert_eq!(BasedNode::get_brain_n(netuid), 5);
        step_block(1);
        ...
        let result =
            BasedNode::set_weights(RuntimeOrigin::signed(U256::from(0)), 1, uids, values, 0);
...
fn test_no_signature() {
    new_test_ext().execute_with(|| {
        ...
        let result = BasedNode::set_weights(RuntimeOrigin::none(), 1, uids, values, 0);
        assert_eq!(result, Err(DispatchError::BadOrigin.into()));
    });
}

// Tests that weights cannot be set BY non-registered computekeys.
#[test]
fn test_set_weights_err_not_active() {
    new_test_ext().execute_with(|| {
        ...
        // Register one agent. Should have uid 0
        register_ok_agent(1, U256::from(666), U256::from(2), 100000);
        BasedNode::get_uid_for_net_and_computekey(netuid, &U256::from(666))
            .expect("Not registered.");

        let weights_keys: Vec<u16> = vec![0]; // Uid 0 is valid.
        let weight_values: Vec<u16> = vec![1];
        // This computekey is NOT registered.
        let result = BasedNode::set_weights(
...
fn test_set_weight_not_enough_values() {
    new_test_ext().execute_with(|| {
        ...
        register_ok_agent(1, account_id, U256::from(2), 100000);
        let agent_uid: u16 = BasedNode::get_uid_for_net_and_computekey(netuid, &U256::from(1))
            .expect("Not registered.");
        BasedNode::set_validator_permit_for_uid(netuid, agent_uid, true);
        BasedNode::set_max_weight_limit(netuid, u16::MAX );

        register_ok_agent(1, U256::from(3), U256::from(4), 300000);
        BasedNode::set_min_allowed_weights(netuid, 2);
...
// Tests that the weights set fails if you pass too many uids for the brain
#[test]
fn test_set_weight_too_many_uids() {
    new_test_ext().execute_with(|| {
        ...
        register_ok_agent(1, U256::from(1), U256::from(2), 100_000);
        let agent_uid: u16 = BasedNode::get_uid_for_net_and_computekey(netuid, &U256::from(1))
            .expect("Not registered.");
        BasedNode::set_validator_permit_for_uid(netuid, agent_uid, true);

        register_ok_agent(1, U256::from(3), U256::from(4), 300_000);
        BasedNode::set_min_allowed_weights(1, 2);
        BasedNode::set_max_weight_limit(netuid, u16::MAX);

        // Should fail because we are setting more weights than there are agents.
        let weight_keys: Vec<u16> = vec![0, 1, 2, 3, 4]; // more uids than agents in brain.
        ...
        // Shouldnt fail because we are setting less weights than there are agents.
        let weight_keys: Vec<u16> = vec![0, 1]; // Only on agents that exist.
        let weight_values: Vec<u16> = vec![10, 10]; // random value.
        assert_ok!(BasedNode::set_weights(
            RuntimeOrigin::signed(U256::from(1)),
            1,
...
fn test_set_weights_sum_larger_than_u16_max() {
    new_test_ext().execute_with(|| {
        ...
        let all_weights: Vec<Vec<I32F32>> = BasedNode::get_weights(netuid);
        let weights_set: &Vec<I32F32> = &all_weights[agent_uid as usize];
...
fn test_check_length_allows_singleton() {
    new_test_ext().execute_with(|| {
        ...
        let expected = true;
        let result = BasedNode::check_length(netuid, uid, &uids, &weights);
...
fn test_check_length_to_few_weights() {
    new_test_ext().execute_with(|| {
        ...
        BasedNode::set_target_registrations_per_interval(netuid, 100);
        BasedNode::set_max_registrations_per_block(netuid, 100);
        // register morw than min allowed
        register_ok_agent(1, U256::from(1), U256::from(1), 300_000);
        register_ok_agent(1, U256::from(2), U256::from(2), 300_001);
        register_ok_agent(1, U256::from(3), U256::from(3), 300_002);
        register_ok_agent(1, U256::from(4), U256::from(4), 300_003);
        register_ok_agent(1, U256::from(5), U256::from(5), 300_004);
        register_ok_agent(1, U256::from(6), U256::from(6), 300_005);
        register_ok_agent(1, U256::from(7), U256::from(7), 300_006);
        BasedNode::set_min_allowed_weights(netuid, min_allowed_weights);
...
fn test_normalize_weights_does_not_mutate_when_sum_is_zero() {
    new_test_ext().execute_with(|| {
        ...
        let result = BasedNode::normalize_weights(weights);
...
fn test_max_weight_limited_allow_self_weights_to_exceed_max_weight_limit() {
    new_test_ext().execute_with(|| {
        ...
        let expected = true;
        let result = BasedNode::max_weight_limited(netuid, uid, &uids, &weights);
...
fn test_is_self_weight_weights_length_not_one() {
    new_test_ext().execute_with(|| {
        ...
        let expected = false;
        let result = BasedNode::is_self_weight(uid, &uids, &weights);
...
fn test_check_len_uids_within_allowed_within_network_pool() {
    new_test_ext().execute_with(|| {
        ...
        register_ok_agent(netuid, U256::from(1), U256::from(1), 0);
        register_ok_agent(netuid, U256::from(3), U256::from(3), 65555);
        register_ok_agent(netuid, U256::from(5), U256::from(5), 75555);
        let max_allowed: u16 = BasedNode::get_brain_n(netuid);
        ...
        let expected = true;
        let result = BasedNode::check_len_uids_within_allowed(netuid, &uids);
        assert_eq!(
            expected, result,
