...
fn test_serving_subscribe_ok_dispatch_info_ok() {
    new_test_ext().execute_with(|| {
        ...
        let call = RuntimeCall::BasedNode(BasednodeCall::serve_brainport {
            netuid,
            version,
            ip,
            port,
...
fn test_serving_ok() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(BasedNode::serve_brainport(
        ...
        let agent = BasedNode::get_brainport_info(netuid, &computekey_account_id);
        assert_eq!(agent.ip, ip);
        assert_eq!(agent.version, version);
        assert_eq!(agent.port, port);
        assert_eq!(agent.ip_type, ip_type);
        assert_eq!(agent.protocol, protocol);
        assert_eq!(agent.placeholder1, placeholder1);
        assert_eq!(agent.placeholder2, placeholder2);
    });
}
...
fn test_serving_set_metadata_update() {
    new_test_ext().execute_with(|| {
        ...
        let agent = BasedNode::get_brainport_info(netuid, &computekey_account_id);
        assert_eq!(agent.ip, ip2);
        assert_eq!(agent.version, version2);
        assert_eq!(agent.port, port2);
        assert_eq!(agent.ip_type, ip_type2);
        assert_eq!(agent.protocol, protocol2);
        assert_eq!(agent.placeholder1, placeholder12);
        assert_eq!(agent.placeholder2, placeholder22);
    });
}

#[test]
#[cfg(not(tarpaulin))]
fn test_brainport_serving_rate_limit_exceeded() {
    new_test_ext().execute_with(|| {
        let computekey_account_id = U256::from(1);
        let netuid: u16 = 1;
        let tempo: u16 = 13;
...
#[test]
fn test_brainport_invalid_port() {
    new_test_ext().execute_with(|| {
        let computekey_account_id = U256::from(1);
        let netuid: u16 = 1;
        let tempo: u16 = 13;
...
fn test_prometheus_serving_subscribe_ok_dispatch_info_ok() {
    new_test_ext().execute_with(|| {
        ...
        let port: u16 = 128;
        let ip_type: u8 = 4;
        let call = RuntimeCall::BasedNode(BasednodeCall::serve_prometheus {
            netuid,
            version,
            ip,
            port,
...
fn test_prometheus_serving_ok() {
    new_test_ext().execute_with(|| {
        ...
        add_network(netuid, tempo, modality);
        register_ok_agent(netuid, computekey_account_id, U256::from(66), 0);
        assert_ok!(BasedNode::serve_prometheus(
            <<Test as Config>::RuntimeOrigin>::signed(computekey_account_id),
            netuid,
            version,
            ip,
        ...
        let agent = BasedNode::get_prometheus_info(netuid, &computekey_account_id);
        assert_eq!(agent.ip, ip);
        assert_eq!(agent.version, version);
        assert_eq!(agent.port, port);
        assert_eq!(agent.ip_type, ip_type);
...
fn test_prometheus_serving_rate_limit_exceeded() {
    new_test_ext().execute_with(|| {
        ...
        BasedNode::set_serving_rate_limit(netuid, 1);
        // Same block, need 1 block to pass
        assert_eq!(
            BasedNode::serve_prometheus(
                <<Test as Config>::RuntimeOrigin>::signed(computekey_account_id),
                netuid,
                version,
                ip,
...
fn test_serving_is_valid_ip_type_ok_ipv4() {
    new_test_ext().execute_with(|| {
        assert_eq!(BasedNode::is_valid_ip_type(4), true);
    });
}
...
fn test_serving_is_valid_ip_type_ok_ipv6() {
    new_test_ext().execute_with(|| {
        assert_eq!(BasedNode::is_valid_ip_type(6), true);
    });
}
...
fn test_serving_is_valid_ip_type_nok() {
    new_test_ext().execute_with(|| {
        assert_eq!(BasedNode::is_valid_ip_type(10), false);
    });
}
...
fn test_serving_is_valid_ip_address_ipv4() {
    new_test_ext().execute_with(|| {
        assert_eq!(
            BasedNode::is_valid_ip_address(4, test::ipv4(8, 8, 8, 8)),
            true
        );
    });
}
...
fn test_serving_is_valid_ip_address_ipv6() {
    new_test_ext().execute_with(|| {
        assert_eq!(
            BasedNode::is_valid_ip_address(6, test::ipv6(1, 2, 3, 4, 5, 6, 7, 8)),
            true
        );
        assert_eq!(
            BasedNode::is_valid_ip_address(6, test::ipv6(1, 2, 3, 4, 5, 6, 7, 8)),
...
fn test_serving_is_invalid_ipv4_address() {
    new_test_ext().execute_with(|| {
        assert_eq!(
            BasedNode::is_valid_ip_address(4, test::ipv4(0, 0, 0, 0)),
            false
        );
        assert_eq!(
            BasedNode::is_valid_ip_address(4, test::ipv4(255, 255, 255, 255)),
            false
        );
        assert_eq!(
            BasedNode::is_valid_ip_address(4, test::ipv4(127, 0, 0, 1)),
            false
        );
        assert_eq!(
            BasedNode::is_valid_ip_address(4, test::ipv6(0xffff, 2, 3, 4, 5, 6, 7, 8)),
            false
        );
    });
}
...
fn test_serving_is_invalid_ipv6_address() {
    new_test_ext().execute_with(|| {
        assert_eq!(
            BasedNode::is_valid_ip_address(6, test::ipv6(0, 0, 0, 0, 0, 0, 0, 0)),
            false
        );
        assert_eq!(
            BasedNode::is_valid_ip_address(
                4,
